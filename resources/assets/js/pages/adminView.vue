    ...
    <v-dialog
      ...
      <div class="modal-open-login">
        <div
          ...
          <img class="icon-close-modal" src="../../img/close_icon.png" alt=""  @click.stop="modalMoreInfo = false;">
          ...
          <div class="input-cuenta-details">
            ...
            <input type="number" class="sinflechas" placeholder="DNI *" v-model="documento" :class="{'input-invalid': errorDocumento}" style="width:100%;" onkeydown="return !(event.keyCode == 69||event.keyCode == 189||event.keyCode == 187||event.keyCode == 190)">
            <input type="number" class="sinflechas" placeholder="Movil *" v-model="movil" :class="{'input-invalid': errorMovil}" style="width:100%;" onkeydown="return !(event.keyCode == 69||event.keyCode == 189||event.keyCode == 187||event.keyCode == 190)">
          </div>
          <div class="">
            <v-btn round class=" elevation-0 pink white--text" :disabled="saveInfoUserDisabled" :loading="saveInfoUserLoading" @click="saveInfoUser();" style="min-width: 174px;">
                GUARDAR
            </v-btn>
            <p v-bind:class="{ errorClass: isError}" class="mt-2">{{msgResponse}}</p>
          </div>
  ...
  export default {
    data: () => ({
      ...
      listMenu: [
        ...
        { name: 'Go & Buy', icon: '/images/iteracion3/icon_go_buy.png', alt: 'icon goandbuy', url:'go-buy-admin' },
        { name: 'Reserva tu locker', icon: '/images/square-check-min.png', alt: 'icon locker', url:'locker' },
        { name: 'Envío Express <span>Envíos inmediatos</span>', icon: '/images/icon_expressa.png', alt: 'icon motorcyrcle', url:'package', subtitulo:true},
      ...
      saveInfoUserLoading:false,
      saveInfoUserDisabled:false
    }),
    props: {
      source: String
    },
    mounted () {
      ...
      this.select_document = this.list_documents[0].name;


    },
    beforeDestroy () {
      console.log("DESTRUIR!");
      EventBus.$off('tarjeta_tokenizada');
   },
   destroyed(){
     console.log("DESTRUIDOOO!");
     EventBus.$off('tarjeta_tokenizada');
   },
    watch: {
      ...
      documento: function (el) {
        if(el.length<=12 && el.length>=8){
      ...
      stateDataUsuario: function(el) {
        if(this.$store.getters.loadStateDataUsuario==2){
    ...
    methods:{
      saveInfoUser: function() {
        ...
        if(vm.documento==''){
          vm.errorDocumento = true;
        }
        if(vm.movil==''){
          vm.errorMovil = true;
        }
        if(vm.documento==''||vm.movil==''){
          vm.msgResponse = "Complete los campos obligatorios *";
          vm.isError = true;
          return;
        }

        if(vm.errorMovil) {
          console.log("ingeso aquiiii");
          vm.msgResponse="Ingrese un teléfono válido";
          vm.isError = true;
          return;
        }
        if(vm.errorDocumento) {
          vm.msgResponse="Ingrese documento válido";
          vm.isError = true;
          return;
        }

        vm.saveInfoUserLoading = true;
        vm.saveInfoUserDisabled = true;
        var pro = localStorage.getItem("user_p_info");
        pro = JSON.parse(pro);

        let usuario_id = pro.id;
        let usuario_id2 = pro.usuario2;
        axios.post('/api/actualizar-user-datos', {
            movil: vm.movil,
            documento: vm.documento,
            tipoDocumento: vm.select_document,
            usuario_id,
            usuario_id2
        })
        .then(function(response){
          ...
          if(res.status == 1){
            vm.msgResponse = "¡Guardado exitosamente!";
            vm.isError = false;

            var pro = localStorage.getItem("user_p_info");
            pro = JSON.parse(pro);
            //alert(res.usuario_dni);
            pro.usuario_dni = res.usuario_dni;
            pro.usuario_movil = res.usuario_movil;
            localStorage.setItem('user_p_info',JSON.stringify(pro));
            vm.saveInfoUserLoading = false;
            setTimeout(()=>{
              vm.modalMoreInfo = false;
              vm.$store.commit('setStateDataUsuario', 'continuar-addcard');
              vm.saveInfoUserDisabled = false;

            },4000);

          } else {
            vm.msgResponse="Hubo un error al guardar. Vuelva a intentarlo.";
            vm.isError = true;
            vm.saveInfoUserLoading = false;
            vm.saveInfoUserDisabled = false;
          }
          ...
          vm.msgResponse="Hubo un error en el envío. Vuelva a intentarlo.";
          vm.isError = true;
          vm.saveInfoUserLoading = false;
          vm.saveInfoUserDisabled = false;
        });
      ...
      activeLink: function (url, index) {
        ...
        if(url=='cuenta'){
          ...
          this.$emit('loadLocalstoredData', 1);
          return;
        }
        if(url=='metodos'){
      ...
      loadTarjetas(src){
        ...
        .then(function(response){
          ...
          if(response.totalregistros){//aqui
            vm.$store.commit('setStateLoadInfoTarjeta',false);
          }
      ...
      loadPromociones:function(){
        ...
        axios({
          ...
          var codigopedidos_raw = response.data.codigopedidos;
          var codigopedidos = {};
          codigopedidos_raw.forEach((el)=>{
            codigopedidos[el.slug]=el.valor
          })
          vm.$store.commit('setStateCodigoPedidos', codigopedidos);
...
.input-cuenta-details input.input-invalid {
  border: solid 0.1rem #ff535a !important;
}
.el-2-12a{
  padding-top: 103px;
}
