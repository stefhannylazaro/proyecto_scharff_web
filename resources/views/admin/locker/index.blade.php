      ...
      <div class="widget widget-fullwidth widget-small">
        <div class="widget-head">
          <div class="tools input-group-btn">
            ...
            <ul class="dropdown-menu pull-right">
              ...
              <li>
                  <a href="{{route('adm.locker.new')}}" class="m-nav__link">
          ...
          <div class="title">Lista de Lockers</div>
        </div>
        ...
        <table id="table1" class="table table-striped table-fw-widget table-hover">
          <thead>
            <tr>
              <th>
                <a href="{{  route('adm.locker.order',['column-1', $order, 'pager' => $pager, 'search' => $search])}}">
                  ID
                  @if($column == 'column-1')
              ...
              <th>
                <a href="{{  route('adm.locker.order',['column-2', $order, 'pager' => $pager, 'search' => $search])}}">
                  Nombre
                  @if($column == 'column-2')
                      <span class="glyphicon glyphicon-sort-by-attributes{{$order == 'desc'? '':'-alt'}}"></span>
                  @endif
              ...
              <th>
                <a href="{{  route('adm.locker.order',['column-3', $order, 'pager' => $pager, 'search' => $search])}}">
                  Dirección
                  @if($column == 'column-3')
                      <span class="glyphicon glyphicon-sort-by-attributes{{$order == 'desc'? '':'-alt'}}"></span>
                  @endif
              ...
              <th>
                <a href="{{  route('adm.locker.order',['column-4', $order, 'pager' => $pager, 'search' => $search])}}">
                  Tamaño
                  @if($column == 'column-4')
                      <span class="glyphicon glyphicon-sort-by-attributes{{$order == 'desc'? '':'-alt'}}"></span>
                  @endif
          ...
          <tbody>
            ...
            <tr class="odd gradeX">
              ...
              <td>
                  <span style="width: 200px;">{{ $model->direccion }}</span>
              </td>

              <td>
                  <span style="width: 200px;">{{ $model->size }}</span>
              </td>
              <td>
                  <span style="width: 200px;">{{ $model->ubicacion_lat.', '.$model->ubicacion_lng }}</span>
              </td>


              <td class="text-center"><a href="{{ route('adm.locker.edit',$model->id) }}" class="ver-detail-item"
                 title="Ver">
