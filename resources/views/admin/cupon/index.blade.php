      ...
      <div class="widget widget-fullwidth widget-small">
        <div class="widget-head">
          <div class="tools input-group-btn">
            ...
            <ul class="dropdown-menu pull-right">
              ...
              <li>
                  <a href="{{route('adm.cupon.new')}}" class="m-nav__link">
        ...
        <table id="table1" class="table table-striped table-fw-widget table-hover">
          <thead>
            <tr>
              <th>
                <a href="{{  route('adm.cupon.order',['column-1', $order, 'pager' => $pager, 'search' => $search])}}">
                  ID
                  @if($column == 'column-1')
              ...
              <th>
                <a href="{{  route('adm.cupon.order',['column-2', $order, 'pager' => $pager, 'search' => $search])}}">
                  Número de usos
                  @if($column == 'column-2')
                      <span class="glyphicon glyphicon-sort-by-attributes{{$order == 'desc'? '':'-alt'}}"></span>
                  @endif
              ...
              <th>
                <a href="{{  route('adm.cupon.order',['column-3', $order, 'pager' => $pager, 'search' => $search])}}">
                  Promoción
                  @if($column == 'column-2')
                      <span class="glyphicon glyphicon-sort-by-attributes{{$order == 'desc'? '':'-alt'}}"></span>
                  @endif
              ...
              <th>
                <a href="{{  route('adm.cupon.order',['column-4', $order, 'pager' => $pager, 'search' => $search])}}">
                  Texto del cupón
                  @if($column == 'column-2')
                      <span class="glyphicon glyphicon-sort-by-attributes{{$order == 'desc'? '':'-alt'}}"></span>
                  @endif
              ...
              <th>
                <a href="{{  route('adm.cupon.order',['column-5', $order, 'pager' => $pager, 'search' => $search])}}">
                  Fecha de creación
                  @if($column == 'column-2')
                      <span class="glyphicon glyphicon-sort-by-attributes{{$order == 'desc'? '':'-alt'}}"></span>
                  @endif
          ...
          <tbody>
            ...
            <tr class="odd gradeX">
              <td>{{ $model->cupon_id }}</td>
              <td>{{ $model->cupon_numero_usos?$model->cupon_numero_usos:0 }}</td>
              <td>
                @if (isset($promociones_ar[$model->promocion_id]))
                    {{ $promociones_ar[$model->promocion_id] }}
                @endif
              </td>
              <td>{{ $model->cupon_texto }}</td>
              <td>{{ $model->created_at }}</td>

              <td class="text-center">
                  <a href="{{ route('adm.cupon.edit',$model->cupon_id) }}" class="ver-detail-item" title="Ver">
