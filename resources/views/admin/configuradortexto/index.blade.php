...
@section('title')
    Administracion | Configurador Textos
@endsection
    ...
    <div class="col-sm-12">
      <div class="widget widget-fullwidth widget-small">
        <div class="widget-head">
          ...
          <div class="title">Lista de Textos</div>
        </div>
        ...
        <table id="table1" class="table table-striped table-fw-widget table-hover">
          <thead>
            <tr>
              <th>
                <a href="{{  route('adm.configtext.order',['column-1', $order, 'pager' => $pager, 'search' => $search])}}">
                Vista
                  @if($column == 'column-1')
              ...
              <th>
                <a href="{{  route('adm.configtext.order',['column-2', $order, 'pager' => $pager, 'search' => $search])}}">
                Clave
                  @if($column == 'column-2')
                      <span class="glyphicon glyphicon-sort-by-attributes{{$order == 'desc'? '':'-alt'}}"></span>
                  @endif
              ...
              <th>
                <a href="{{  route('adm.configtext.order',['column-3', $order, 'pager' => $pager, 'search' => $search])}}">
                Título
                  @if($column == 'column-3')
                      <span class="glyphicon glyphicon-sort-by-attributes{{$order == 'desc'? '':'-alt'}}"></span>
                  @endif
              ...
              <th>
                <a href="{{  route('adm.configtext.order',['column-4', $order, 'pager' => $pager, 'search' => $search])}}">
                Descripción
                  @if($column == 'column-4')
                      <span class="glyphicon glyphicon-sort-by-attributes{{$order == 'desc'? '':'-alt'}}"></span>
                  @endif
          ...
          <tbody>
            ...
            <tr class="odd gradeX">
              ...
              <td>{{ $model->clave }}</td>
              <td>{{ $model->titulo }}</td>
              <td>{{ substr($model->descripcion, 0, 400) }}...</td>
              <td class="text-center"><a href="{{ route('adm.configtext.edit',$model->id) }}" class="ver-detail-item"
                 title="Ver">
