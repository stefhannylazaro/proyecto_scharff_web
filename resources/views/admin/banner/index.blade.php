    ...
    <div class="col-sm-12">
      <div class="widget widget-fullwidth widget-small">
        <div class="widget-head">
          ...
          <div class="title">Lista de Banners</div>
        </div>
        ...
        <table id="table1" class="table table-striped table-fw-widget table-hover">
          <thead>
            <tr>
              <th>
                <a href="{{  route('adm.banner.order',['column-1', $order, 'pager' => $pager, 'search' => $search])}}">
                  Título
                  @if($column == 'column-1')
              ...
              <th>
                <a href="{{  route('adm.banner.order',['column-2', $order, 'pager' => $pager, 'search' => $search])}}">
                  Sub Titulo
                  @if($column == 'column-2')
                      <span class="glyphicon glyphicon-sort-by-attributes{{$order == 'desc'? '':'-alt'}}"></span>
                  @endif
              ...
              <th>
                <a href="{{  route('adm.banner.order',['column-3', $order, 'pager' => $pager, 'search' => $search])}}">
                  Slug
                  @if($column == 'column-3')
                      <span class="glyphicon glyphicon-sort-by-attributes{{$order == 'desc'? '':'-alt'}}"></span>
                  @endif
              ...
              <th>
                <a href="{{  route('adm.banner.order',['column-4', $order, 'pager' => $pager, 'search' => $search])}}">
                  Fecha Creación
                  @if($column == 'column-4')
                      <span class="glyphicon glyphicon-sort-by-attributes{{$order == 'desc'? '':'-alt'}}"></span>
                  @endif
          ...
          <tbody>
            ...
            <tr class="odd gradeX">
              <td>{{ $model->titulo }}</td>
              <td>{{ $model->subtitulo }}</td>
              <td>{{ $model->slug }}</td>
              <td>{{ $model->created_at }}</td>
              <td class="text-center">
                <a href="{{ route('adm.banner.edit',$model->id) }}" class="ver-detail-item"
                 title="Ver">
                  <span class="icon s7-look"></span>
                </a>&nbsp
                <!-- @if($model->estado==1)
                  <input type="checkbox" checked style="vertical-align: top;" name="" value="" class="active-b" data-id="">
                @else
                  <input type="checkbox" style="vertical-align: top;" name="" value="" class="active-b">
                @endif
                <span hidden>{{ $model->id }}</span>
                <span class="infoCheck" style="display:none;"></span> -->
              </td>
