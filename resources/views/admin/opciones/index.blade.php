      ...
      <div class="widget widget-fullwidth widget-small">
        ...
        <table id="table1" class="table table-striped table-fw-widget table-hover">
          <thead>
            <tr>
              <th>
                <a href="{{  route('adm.opciones.order',['column-1', $order, 'pager' => $pager, 'search' => $search])}}">
                  Opción
                  @if($column == 'column-1')
              ...
              <th>
                <a href="{{  route('adm.opciones.order',['column-2', $order, 'pager' => $pager, 'search' => $search])}}">
                  Valor
                  @if($column == 'column-2')
                      <span class="glyphicon glyphicon-sort-by-attributes{{$order == 'desc'? '':'-alt'}}"></span>
                  @endif
            ...
            <tr class="odd gradeX">
              ...
              <td>
                <span style="width: 200px;">
                  ...
                  {{ $model->valor === "1" ? "Activado" : "Desactivado"}}
                  @elseif($model->tipo==='opcion')
                    ...
                    @endphp
                      {{-- {{print_r($obj_opciones)}} --}}

                    @if (isset($obj['opciones']))
                        @php
                          $obj_opciones = $obj['opciones'];
                        @endphp
                        {{$obj_opciones[$model->valor]}}
                    @endif

                  @elseif($model->tipo==='textfield')
                    {{$model->valor}}
                  @endif
                </span>
              </td>
              <td class="text-center"><a href="{{ route('adm.opciones.edit',$model->id) }}" class="ver-detail-item"
                 title="Ver">
