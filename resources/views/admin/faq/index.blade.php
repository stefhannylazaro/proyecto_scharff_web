      ...
      <div class="widget widget-fullwidth widget-small">
        <div class="widget-head">
          <div class="tools input-group-btn">
            ...
            <ul class="dropdown-menu pull-right">
              ...
              <li>
                  <a href="{{route('adm.faq.new')}}" class="m-nav__link">
        ...
        <table id="table1" class="table table-striped table-fw-widget table-hover">
          <thead>
              <tr>
                <th>
                <a href="{{  route('adm.faq.order',['column-1', $order, 'pager' => $pager, 'search' => $search])}}">
                  ID
                  @if($column == 'column-1')
              ...
              <th>
              <a href="{{  route('adm.faq.order',['column-2', $order, 'pager' => $pager, 'search' => $search])}}">
                Categoría
                  @if($column == 'column-2')
                      <span class="glyphicon glyphicon-sort-by-attributes{{$order == 'desc'? '':'-alt'}}"></span>
                  @endif
              ...
              <th>
              <a href="{{  route('adm.faq.order',['column-3', $order, 'pager' => $pager, 'search' => $search])}}">
                Pregunta
                  @if($column == 'column-3')
                      <span class="glyphicon glyphicon-sort-by-attributes{{$order == 'desc'? '':'-alt'}}"></span>
                  @endif
              ...
              <th>
              <a href="{{  route('adm.faq.order',['column-4', $order, 'pager' => $pager, 'search' => $search])}}">
                Respuesta
                  @if($column == 'column-4')
                      <span class="glyphicon glyphicon-sort-by-attributes{{$order == 'desc'? '':'-alt'}}"></span>
                  @endif
              ...
              <th>
              <a href="{{  route('adm.faq.order',['column-5', $order, 'pager' => $pager, 'search' => $search])}}">
                Fecha
                  @if($column == 'column-5')
                      <span class="glyphicon glyphicon-sort-by-attributes{{$order == 'desc'? '':'-alt'}}"></span>
                  @endif
          ...
          <tbody>
            ...
            <tr class="odd gradeX">
              <td>{{ $model->id }}</td>
              <td>{{ $assoc_cats[$model->categoria] }}</td>
              <td>{{ $model->pregunta }}</td>
              <td>{{ $model->respuesta }}</td>
              <td>{{ $model->created_at }}</td>
              <td class="text-center">
                <a href="{{ route('adm.faq.edit',$model->id) }}" class="ver-detail-item"
                 title="Ver">
