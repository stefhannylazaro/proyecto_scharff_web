...
var App = (function () {
  ...
  function sidebarSwipe(){
    ...
    $( document ).ready(function() {
      ...
      /***Calcular costo real***/
      $('.btn-costo-real').on('click', function(){
        $('#msg-pc').text('');
        if($('#gobuy_pesoconsolidado').val()!=="") {
          if($('#gobuy_valorreal').val()!=="" && $('#gobuy_pesoreal').val()!==""){
            var $this=this;
            var costo=$($this).next().text();
            $('#gobuy_costoreal').val();
            $('#gobuy_montoextra').val();
            $.ajax({
              url : '/api/costo-real',
              data : {
                 costo_cobrado: costo,
                 valor_real: $('#gobuy_valorreal').val(),
                 peso_real: $('#gobuy_pesoreal').val(),
                 //peso_consolidado: $('#gobuy_pesoconsolidado').val(),
                 impuesto: $('#gobuy_impuesto').val()
              },
              type : 'post',
              dataType : 'json',
              ...
              success : function(data) {
                if(data.status==1) {
                  console.log(data);
                  $('#gobuy_costoreal').val(data.data.costo_real);
                  $('#gobuy_montoextra').val(data.data.saldo);
                  $('#monto-extra').val(data.data.saldo);
                }
              },
            });
          }
        } else {
          //*Debe ingresar peso consolidado.
          $('#msg-pc').text('*Debe ingresar peso consolidado');
        }
      ...
      if(pathAux[2]=='gobuypedido'){
        $('.switch-pedido').addClass('hide');
        if(pathAux[3]!=undefined){
            $('.th-acciones').removeClass('hide');
            $('.td-acciones').removeClass('hide');
            if(pathAux[4]==0) {
              $('#item-0').addClass('active');
            } else {
              if(pathAux[4]==1) {//recibido
                $('#item-1').addClass('active');
                $('.th-check-cons').removeClass('hide');
              } else {
                if(pathAux[4]==2) {//transito
                  $('#item-2').addClass('active');
                  $('.switch-pedido').removeClass('hide');
                  $('.th-acciones').addClass('hide');
                  $('.th-consolidado').removeClass('hide');
                } else {
                  if(pathAux[4]==3) {
            ...
            $('.th-acciones').addClass('hide');
            $('.td-acciones').addClass('hide');
        }

      }
      if(pathAux[pathAux.length-1]=='consolidado') {
        $('.check-consolidado').attr( 'checked', true );
        $('.title-check').text("Lista consolidados");

        $('#item-cons').addClass('active');
      } else {
        $('.check-consolidado').attr( 'checked', false );
        $('.title-check').text("Lista de pedidos");

        $('#item-pedido').addClass('active');
      }
      /**************end active menu**************/

      /*************CAMBIO DE ESTADO INDIVIDUAL*************/
      $('.btn-state-modal').on('click', function(){
        $('.infoState1').text('');
        var $this=this;
        var id=$($this).parent().find('.id-pedido').text();
        var state=$($this).data('state');
        $("#modal-tab-recibido").data('data-id',id);
        $("#modal-tab-recibido").data('data-state',state);
        console.log(id);
        var textState='';
        switch (state) {
          case 0:
            textState='Registrado';
            break;
          case 1:
            textState='Recibido en Miami';
            break;
          case 2:
            textState='En tránsito';
            break;
          case 3:
            textState='En almacén Scharff (Lima)';
            break;
          case 4:
            textState='Entregado';
            break;
        }
        $(".text-state").text(textState);
        $("#modal-tab-recibido").modal();
      });

      $('.btn-state-modal-group').on('click', function(){
        var $this=this;
        var id=$($this).parent().find('.id-pedido').text();
        var state=$($this).data('state');
        $("#modal-change-status-cons").data('data-id',id);
        $("#modal-change-status-cons").data('data-state',state);
        ...
        $(".text-state").text(textState);
        $("#modal-change-status-cons").modal();
      });


      $('.btn-consolidado-modal').on('click', function(){
        var textCodigos='';
        var textPedido1='';
        var textPedido2='';
        itemsCheck.forEach(function(element) {
          if(itemsCheck.length==1){
            textCodigos=textCodigos+element;
          } else {
            textCodigos=textCodigos+element+', ';
          }
        });
        ...
        if(itemsCheck.length==1){
          textPedido1 ="el pedido";
          textPedido2 ="El pedido cambiará";
        } else {
          if(itemsCheck.length>1){
            textPedido1 ="los pedidos";
            textPedido2 ="Todos los pedidos cambiarán";
          }
        }
        $('.text-pedidos-1').text(textPedido1);
        $('.text-pedidos-2').text(textPedido2);
        $('.code-pedidos').text(textCodigos);
        $("#modal-consolidado").modal();
      });
      $('.btn-update-state').on('click', function(){
        var item=[];
        var $this=this;
        $('.infoState1').text('');
        var codeState=$("#modal-tab-recibido").data('data-state');
        console.log(codeState);
        item.push($("#modal-tab-recibido").data('data-id'));
        $.ajax({
          ...
          dataType : 'json',
          beforeSend: function(){
            $('.btn-update-state').prepend('<i class="fa fa-spinner fa-spin"></i>&nbsp;').attr("disabled",true);
          },
          success : function(data) {
            // $(".btn-update-state").attr("disabled",false);
            // $(".btn-update-state i").remove();
            if(data.status==1) {
              $('.infoState1').text("Cambio de estado exitoso.");
              $('.infoState1').fadeIn("slow", function() {});
              setTimeout(function(){
                $('.infoState1').fadeOut("slow", function() {});
                location.reload();
              }, 300);
            } else {
              $('.btn-update-state').attr("disabled",false);
              $('.btn-update-state i').remove();
              $('.infoState1').text(data.mensajes[0]);
            }
          },
        });
      });
      ////
      $('.btn-update-state-by-cons').on('click', function(){
        var $this=this;
        $('.infoState1').text("");
        console.log($("#modal-change-status-cons").data('data-id'));//
        console.log($("#modal-change-status-cons").data('data-state'));//
        $.ajax({
          url : '/api/change-status-byconsolidado',
          data : {
             id : $("#modal-change-status-cons").data('data-id'),
             estado: $("#modal-change-status-cons").data('data-state')
          },
          type : 'post',
          dataType : 'json',
          beforeSend: function(){
            $('.btn-update-state-by-cons').prepend('<i class="fa fa-spinner fa-spin"></i>').attr("disabled",true);
          },
          success : function(data) {
            if(data.status==1) {
              $('.infoState1').html(data.mensajes[0]);
              $('.infoState1').fadeIn("slow", function() {});
              setTimeout(function(){
                $('.infoState1').fadeOut("slow", function() {});
                location.reload();
              ...
              $(".btn-update-state-by-cons").attr("disabled",false);
              $(".btn-update-state-by-cons i").remove();
              $('.infoState1').html(data.mensajes[0]);
            }
          },
        });
      });
      /*************FIN CAMBIO DE ESTADO A ENTREGADO*************/

      /*************CAMBIO CONSOLIDADO*************/
      $('.btn-state-modal-cons').on('click', function(){
        var $this=this;
        var id=$($this).parent().find('.id-consolidado').text();
        var state=$($this).data('state');
        $("#modal-consolidado-m").data('data-id',id);
        $("#modal-consolidado-m").data('data-state',state);
        $("#modal-consolidado-m").modal();
      });
      $('.btn-update-consolidado').on('click', function(){
        console.log('estado pedido');
        console.log($("#modal-consolidado-m").data('data-state'));
        console.log('id consolidado');
        console.log($("#modal-consolidado-m").data('data-id'));

        $.ajax({
          url : '/api/update-consolidado',
          data : {
             id : $("#modal-consolidado-m").data('data-id'),
             estado: $("#modal-consolidado-m").data('data-state')
          },
          type : 'post',
          dataType : 'json',
          beforeSend: function(){
            $('.btn-update-consolidado').prepend('<i class="fa fa-spinner fa-spin"></i>&nbsp;').attr("disabled",true);
          },
          success : function(data) {
            if(data.status==1) {
              $('.infoStateCons').text("Cambio de estado exitoso.");
              $('.infoStateCons').fadeIn("slow", function() {});
              setTimeout(function(){
                $('.infoStateCons').fadeOut("slow", function() {});
                location.reload();
              }, 500);
            } else {
              $('.btn-update-consolidado').attr("disabled",false);
              $('.btn-update-consolidado i').remove();
              $('.infoStateCons').text("Hubo un error, vuelva a intentarlo.");
            }
          },
        });
      });
      /*************FIN CAMBIO CONSOLIDADO*************/

      $('.active-b').change(function() {
          console.log("asd");
          var $this= this;
          $('.active-b').val($this.checked);
          var idB=$($this).next().text();;
          $.ajax({
            url : '/api/active',
            data : { id : idB },
            type : 'post',
            ...
            success : function(data) {
              if(data.status) {
                if($this.checked) {
                  $($this).parent().find('.infoCheck').text('Activado');
                } else {
                  $($this).parent().find('.infoCheck').text('Desactivado');
                }
                $($this).parent().find('.infoCheck').fadeIn("slow", function() {});
                setTimeout(function(){
                  $($this).parent().find('.infoCheck').fadeOut("slow", function() {});
                }, 2000);

              } else {
                $($this).parent().find('.infoCheck').text(data.mensaje);
              }
            },
          });
      });
      ...
      $('.change-status').change(function() {
          var $this= this;
          $('.change-status').val($this.checked);
        ...
        if(items.length) {
          ...
          $.ajax({
            ...
            success : function(data) {
              ...
              if(data.status==1) {
                items=[];
                $('.infoChangeStatus').text("Datos actualizados!");
                $('.infoChangeStatus').fadeIn("slow", function() {});
                setTimeout(function(){
                  $('.infoChangeStatus').fadeOut("slow", function() {});
                  location.reload();
                }, 2000);
              } else {
                $($this).parent().find('.infoCheck').text(data.mensajes);
              }
            },
          });
        }
      });
      /****************END CAMBIAR ESTADO MASIVO******************/

      /*************PANEL ENTREGADO*******************/

      $('.change-status-type').change(function() {
          var $this= this;
          var id=$($this).parent().find('.id-pedido').text();
          if( $($this).is(':checked') ){
            itemsCheck.push(id);
          } else {
            var index = itemsCheck.indexOf(id);
            if (index > -1) {
              itemsCheck.splice(index, 1);
            }
          }
          if(itemsCheck.length>0){
            $('.btn-consolidado-modal').removeClass('hide');
          } else {
            $('.btn-consolidado-modal').addClass('hide');
          }
          console.log(itemsCheck);
      });
      $('.check-consolidado').change(function() {
          var $this= this;
          var path='';
          if( $($this).is(':checked') ){
            path=window.location.pathname+'/consolidado';
          } else {
            path = window.location.pathname.replace('/consolidado', '');
          }
          location.href=path;
      });

      $('.btn-create-consolidado').on('click', function(){
          var $this= this;
          //1. Cambia a estado en transito
          $.ajax({
            url : '/api/change-status',
            data : {
               list : itemsCheck,
            ...
            success : function(data) {
              if(data.status==1) {
                //2. Crear un consolidado
                $.ajax({
                  url : '/api/save-consolidado',
                  data : {
                     list : itemsCheck
                  },
                  type : 'post',
                  dataType : 'json',
                  beforeSend: function(){
                    $('.btn-create-consolidado').prepend('<i class="fa fa-spinner fa-spin"></i>&nbsp;').attr("disabled",true);
                  },
                  success : function(data) {
                    if(data.status==1) {
                      $('.infoState2').text("Se guardo con éxito.");
                      $('.infoState2').fadeIn("slow", function() {});
                      setTimeout(function(){
                        $('.infoState2').fadeOut("slow", function() {});
                        location.reload();
                      }, 300);

                    } else {
                      $('.infoState2').text("Hubo un error, vuelva a intentarlo.");
                    }
                  },
                });
              } else {
  ...
  function resetValue() {
    $('.reset-locker').on('click',function(){
      //console.log("aqui");
      var $this= $(this);
      var idAux;
      //$this.attr('id')== 'locker'? idAux=6 : idAux=3;
      switch ($this.attr('id')) {
        case 'locker':
          idAux=6;
          break;
      ...
      $.ajax({
        ...
        success : function(data) {
          if(data.status) {
            $this.parent().parent().parent().find('tbody').find('tr').first().find('td:eq(1)').text(1);
            $this.parent().next().find('.infoReset').fadeIn("slow", function() {});
            $this.parent().next().find('.infoReset').text(data.mensaje);
            setTimeout(function(){
              $this.parent().next().find('.infoReset').fadeOut("slow", function() {});
            }, 2000);
          } else {
            $this.parent().next().find('.infoReset').text(data.mensaje);
          }
